...
func (canvas *Canvas) SwSetTarget(stride uint, width uint, height uint, cs ColorSpace) error {
	...
	result := C.tvg_swcanvas_set_target(canvas.canvas, (*C.uint32_t)(unsafe.Pointer(&canvas.buffer[0])),
		C.uint32_t(stride), C.uint32_t(width), C.uint32_t(height), C.Tvg_Colorspace(cs))
...
It accepts an optional parameter @p op to choose between different rendering engine behaviors.
	...
	@since 1.0
*/
func GlCanvasCreate(op EngineOption) Canvas {
	return Canvas{
		canvas: C.tvg_glcanvas_create(C.Tvg_Engine_Option(op)),
	}
}
...
func (canvas *Canvas) GlSetTarget(
	...
	result := C.tvg_glcanvas_set_target(canvas.canvas, display, surface, context, C.int32_t(id),
		C.uint32_t(width), C.uint32_t(height), C.Tvg_Colorspace(colorSpace))
	return resultError(result)
}

/*
WgCanvasCreate creates a new WebGPU Canvas object with optional rendering engine settings.
...
It accepts an optional parameter @p op to choose between different rendering engine behaviors.
	...
	@since 1.0
*/
func WgCanvasCreate(op EngineOption) Canvas {
	return Canvas{
		canvas: C.tvg_wgcanvas_create(C.Tvg_Engine_Option(op)),
	}
}
...
func (canvas *Canvas) WgSetTarget(
	...
	width uint, height uint, colorSpace ColorSpace, typ int,
) error {
	result := C.tvg_wgcanvas_set_target(canvas.canvas, device, instance, target,
		C.uint32_t(width), C.uint32_t(height), C.Tvg_Colorspace(colorSpace), C.int32_t(typ))
	return resultError(result)
}
...
func (canvas *Canvas) Destroy() error {
	canvas.bufferPinner.Unpin()
	canvas.buffer = nil
	result := C.tvg_canvas_destroy(canvas.canvas)
	return resultError(result)
}
...
func (canvas Canvas) Add(paint Paint) error {
	result := C.tvg_canvas_add(canvas.canvas, paint.paint())
	return resultError(result)
}
...
func (canvas Canvas) Insert(target Paint, paint Paint) error {
	...
	result := C.tvg_canvas_insert(canvas.canvas, cTarget, paint.paint())
	return resultError(result)
}
...
func (canvas Canvas) Remove(paint Paint) error {
	result := C.tvg_canvas_remove(canvas.canvas, paint.paint())
	return resultError(result)
}
...
func (canvas Canvas) Update(Paint) error {
	result := C.tvg_canvas_update(canvas.canvas)
	return resultError(result)
}
...
func (canvas Canvas) Draw(clear_ bool) error {
	result := C.tvg_canvas_draw(canvas.canvas, C.bool(clear_))
	return resultError(result)
}
...
func (canvas Canvas) Sync() error {
	result := C.tvg_canvas_sync(canvas.canvas)
	return resultError(result)
}
...
func (canvas Canvas) SetViewport(x int, y int, width int, height int) error {
	result := C.tvg_canvas_set_viewport(canvas.canvas, C.int32_t(x), C.int32_t(y), C.int32_t(width), C.int32_t(height))
	return resultError(result)
}
