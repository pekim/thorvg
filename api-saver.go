...
func (saver Saver) SavePaint(paint Paint, path string, quality uint) error {
	...
	result := C.tvg_saver_save_paint(saver.saver, paint.paint(), cPath, C.uint32_t(quality))
...
func (saver Saver) SaveAnimation(animation Animation, path string, quality uint, fps uint) error {
	...
	result := C.tvg_saver_save_animation(saver.saver, animation.animation, cPath, C.uint32_t(quality), C.uint32_t(fps))
	return resultError(result)
}
...
func (saver Saver) Sync() error {
	result := C.tvg_saver_sync(saver.saver)
	return resultError(result)
}
...
func (saver Saver) Del() error {
	result := C.tvg_saver_del(saver.saver)
	return resultError(result)
}
