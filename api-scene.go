...
func (scene Scene) Insert(target Paint, at Paint) error {
	result := C.tvg_scene_insert(scene.paint_, target.paint(), at.paint())
	return resultError(result)
}
...
func (scene Scene) Remove(paint Paint) error {
	result := C.tvg_scene_remove(scene.paint_, paint.paint())
	return resultError(result)
}
...
func (scene Scene) ClearEffects() error {
	result := C.tvg_scene_clear_effects(scene.paint_)
	return resultError(result)
}
...
func (scene Scene) AddEffectGaussianBlur(sigma float64, direction int, border int, quality int) error {
	result := C.tvg_scene_add_effect_gaussian_blur(
		scene.paint_, C.double(sigma),
		C.int32_t(direction), C.int32_t(border), C.int32_t(quality),
	)
	return resultError(result)
}
...
func (scene Scene) AddEffectDropShadow(
	...
	result := C.tvg_scene_add_effect_drop_shadow(scene.paint_,
		C.int32_t(r), C.int32_t(g), C.int32_t(b), C.int32_t(a),
		C.double(angle), C.double(distance), C.double(sigma), C.int32_t(quality),
	)
	return resultError(result)
}
...
func (scene Scene) AddEffectFill(r int, g int, b int, a int) error {
	result := C.tvg_scene_add_effect_fill(scene.paint_, C.int32_t(r), C.int32_t(g), C.int32_t(b), C.int32_t(a))
	return resultError(result)
}
...
func (scene Scene) AddEffectTint(
	...
	result := C.tvg_scene_add_effect_tint(scene.paint_,
		C.int32_t(black_r), C.int32_t(black_g), C.int32_t(black_b),
		C.int32_t(white_r), C.int32_t(white_g), C.int32_t(white_b),
		C.double(intensity),
	)
	return resultError(result)
}
...
func (scene Scene) AddEffectTritone(
	...
	result := C.tvg_scene_add_effect_tritone(scene.paint_,
		C.int32_t(shadow_r), C.int32_t(shadow_g), C.int32_t(shadow_b),
		C.int32_t(midtone_r), C.int32_t(midtone_g), C.int32_t(midtone_b),
		C.int32_t(highlight_r), C.int32_t(highlight_g), C.int32_t(highlight_b),
		C.int32_t(blend),
	)
	return resultError(result)
}
