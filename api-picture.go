...
func (picture Picture) Load(path string) error {
	...
	result := C.tvg_picture_load(picture.paint_, cPath)
...
func (picture Picture) LoadRaw(data []byte, width uint, height uint, colorSpace ColorSpace) error {
	result := C.tvg_picture_load_raw(picture.paint_, (*C.uint32_t)(unsafe.Pointer(&data[0])),
		C.uint32_t(width), C.uint32_t(height), C.Tvg_Colorspace(colorSpace), true)
	return resultError(result)
}
...
func (picture Picture) LoadDataText(text string, mimetype string, rpath string) error {
	...
	result := C.tvg_picture_load_data(picture.paint_, (*C.char)(unsafe.Pointer(unsafe.StringData(text))), C.uint32_t(len(text)),
		cMimetype, cRpath, true)
	return resultError(result)
}
...
func (picture Picture) SetAssetResolver(resolver PictureAssetResolver) error {
	goPictureAssetResolver = resolver
	result := C.tvg_picture_set_asset_resolver(picture.paint_, (*[0]byte)(C.c_picture_asset_resolver), nil)
	return resultError(result)
}

//export go_picture_asset_resolver
func go_picture_asset_resolver(cPaint C.Tvg_Paint, src *C.char, _data unsafe.Pointer) C.bool {
	...
	return C.bool(goPictureAssetResolver(paint, C.GoString(src)))
}

var goPictureAssetResolver PictureAssetResolver
...
func (picture Picture) SetSize(width float32, height float32) error {
	result := C.tvg_picture_set_size(picture.paint_, C.float(width), C.float(height))
	return resultError(result)
}
...
func (picture Picture) GetSize() (float32, float32, error) {
	var width C.float
	var height C.float
	result := C.tvg_picture_get_size(picture.paint_, &width, &height)
	return float32(width), float32(height), resultError(result)
}
...
func (picture Picture) SetOrigin(x float32, y float32) error {
	result := C.tvg_picture_set_origin(picture.paint_, C.float(x), C.float(y))
	return resultError(result)
}
...
func (picture Picture) GetOrigin() (float32, float32, error) {
	var x C.float
	var y C.float
	result := C.tvg_picture_get_origin(picture.paint_, &x, &y)
	return float32(x), float32(y), resultError(result)
}
...
func (picture Picture) GetPaint(id uint) (Paint, bool) {
	paint := C.tvg_picture_get_paint(picture.paint_, C.uint32_t(id))
